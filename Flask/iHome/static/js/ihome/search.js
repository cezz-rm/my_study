...
//    将获取的开始时间结束时间增加到url中去
    var sd = location.search.split('&')[1].split('=')[1];
    var ed = location.search.split('&')[2].split('=')[1];
    var path = location.search.replace('sd=' + sd, 'sd=' + startDate).replace('ed=' + ed, 'ed=' + endDate);
...
$(document).ready(function(){
    ...
    $(".filter-item-bar>.filter-area").on("click", "li", function(e) {
        ...
        if (!$(this).hasClass("active")) {
            ...
            $(this).parents(".filter-area").removeClass("active");
            $('.display-mask').css('display', 'None');
            // 点击地区后修改url信息并请求
            var new_aid = $(this).attr('area-id');
            var aid = location.search.split('&')[0].split('=')[1];
            var path = location.search.replace('aid=' + aid, 'aid=' + new_aid);

            // 修改url的参数但是不刷新页面
            window.history.pushState(null, null, path);
            $.ajax({
                url: '/house/allsearch/' + path,
                type: 'get',
                dataType: 'json',
                success: function (data) {
                    if (data.code == '200'){
                        var house_info_html = template('house_info_search', {hlist: data.hlist});
                        $('.house-list').html(house_info_html);

                        var area_html = template('area_list', {alist: data.alist});
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function(e) {
        ...
        if (!$(this).hasClass("active")) {
            ...
            var path = location.search.split('&sk=')[0] + '&sk=' + sort_key;
...
$(document).ready(function () {
    ...
    $.get('/house/allsearch/' + path, function (data) {
       if (data.code == '200'){
           ...
           // $(".filter-title-bar>.filter-title").eq(1).children("span").eq(0).html(data.area_name);
